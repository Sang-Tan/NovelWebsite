...
<form method="post" class="containter-fluid ml-5 mr-5" enctype="multipart/form-data"
    ...
    <div class=" d-flex align-items-center mb-2">
        ...
        <input class="input-text" style="flex-grow: 1" type="text" name="novel_name" id="novel_name"
               placeholder="Tên truyện" value="${reqNovel.name}" ${submitAllowed? "" : "disabled"} required>
    </div>
    <div class="d-flex align-items-center mb-2">
            ...
            <c:forEach items="${genres}" var="genre">
                <div class="checkbox-holder">
                    <input class="checkbox1" type="checkbox" data-genre="${genre.id}"
                           id="genre${genre.id}" ${submitAllowed? "" : "disabled"}
                           <c:if test="${novelGenreIds.contains(genre.id)}">checked</c:if>>
    ...
    <div class="d-flex align-items-start mb-2">
        ...
        ${submitAllowed? "" : "disabled"} required>${reqNovel.summary}</textarea>
    </div>
    <div class="d-flex align-items-start mb-4">
        ...
        <select class="input-text" name="status" id="status" style="flex-grow: 1;" ${submitAllowed? "" : "disabled"}>
